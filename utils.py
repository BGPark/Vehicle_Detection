...
def find_matches(img, uris):
    ...
    # see also http://opencv-python-tutroals.readthedocs.io/en/latest/py_tutorials/py_imgproc/py_template_matching/py_template_matching.html
    method = cv2.TM_CCOEFF_NORMED
    # Iterate through template list
    for uri in uris:
        # Read in templates one by one
        tmp = mpimg.imread(uri)
...
def color_hist(img, nbins=32, bins_range=(0, 256)):
    ...
    return hist_features, ch1, ch2, ch3, bin_centers
...
def extract_hist_features(imgs, cspace='RGB', spatial_size=(32, 32),
    ...
    for img in imgs:
        ...
        spatial_features = bin_spatial(image, cspace, spatial_size)
        # Apply color_hist() to get color histogram features
        hist_features, rhist, ghist, bhist, bin_centers = color_hist(image, hist_bins, hist_range)
...
def get_data_and_info(uri, recursive=True):
    images = glob.glob(uri, recursive=recursive)
    ...
    dict = data_look(cars, notcars)
    return cars, notcars, dict
...
def extract_features(imgs, color_space='RGB', spatial_size=(32, 32),
                        ...
                        pix_per_cell=8, cell_per_block=2, hog_channel=0,
                        spatial_feat=True, hist_feat=True, hog_feat=True, isURI=True):
    # Create a list to append feature vectors to
    features = []
    # Iterate through the list of images
    for file in imgs:
...
def find_cars(img, xstart, xstop, ystart, ystop, scale, svc, X_scaler, orient, pix_per_cell, cell_per_block, spatial_size, hist_bins):
    ...
    for xb in range(nxsteps):
        for yb in range(nysteps):
            ...
            # print('bin spatial = %f' % (time.time() - t1))

            # t1 = time.time()
            hist_features, _, _, _, _ = color_hist(subimg, nbins=hist_bins)
            # print('color hist = %f' % (time.time() - t1))
            ...
            # print('prediction = %f' % (time.time() - t1))
            ...
            # if True:
                ...
                windows.append(((xbox_left, ytop_draw),(xbox_left + win_draw, ytop_draw + win_draw)))
                # print(windows[-1])
    return windows